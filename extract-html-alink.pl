...
if($#ARGV < 0)
{
    print("リンク抽出スクリプト\n\n".
        "使用法\n".
        " ".basename($0)." ([a|img]) [url_list_file.txt|url] ([domain|+|-] [path])\n\n".
...
foreach my $url (@arr_url) {
    ...
    my ($scheme_base, $auth_base, $path_base, $query_base, $frag_base) = URI::Split::uri_split($url);
    ...
    my $p = HTML::LinkExtor->new(
                sub {
                    ...
                    if(lc($tag) eq (($mode eq 'a')?'a':'img') && defined(($mode eq 'a')?$links{'href'}:$links{'src'}) && length(($mode eq 'a')?$links{'href'}:$links{'src'})>0){
                        ...
                        if(defined($scheme) && lc($scheme) ne 'http'){  } # http:// 以外
                        ...
                        elsif(!defined($auth) && (!defined($path) || length($path)<=0)){  } # urlで無い可能性
                        elsif(defined($auth_user) && $auth_user eq '+' && defined($auth) && (lc($auth) ne lc($auth_base))){  } # 他のドメインは無視
                        elsif(defined($auth_user) && $auth_user ne '+' && defined($auth) && ($auth !~ m/$auth_user/)){  } # 指定ドメイン以外
                        elsif(defined($auth_user) && $auth_user ne '+' && !defined($auth) && ($auth_base !~ m/$auth_user/)){  } # 指定ドメイン以外
                            ...
                            if(!defined($scheme) && !defined($auth) && $path !~ /^\//){
                                # 相対ディレクトリの場合
                                $href = 'http://'.$auth_base.substr($path_base,0,rindex($path_base,'/')+1).$href;
                            }
                            elsif(!defined($scheme) && !defined($auth)){ $href = 'http://'.$auth_base.$href; }    # 絶対ディレクトリの場合
                            # 完成した抽出リンクを、一旦配列に格納
